...
var getFunctionValue = (value, target, index, total, store) => {
  ...
  if (isFnc(value)) {
    ...
    func = () => {
      ...
      let computed = (_a = getComputedStyle(target)) == null ? void 0 : _a.getPropertyValue(cssVarName);
...
var JSAnimation = class extends Timer {
  constructor(targets, parameters, parent, parentPosition, fastSet = false, index = 0, length = 0) {
    ...
    const hasSpring = !isUnd(animEase.ease);
      ...
      for (let p in params) {
        if (isKey(p)) {
          ...
          for (let l = keyframes.length; tweenIndex < l; tweenIndex++) {
            ...
            const tweenEasing = hasSpring2 ? keyEasing.ease : keyEasing;
            const tweenDuration = hasSpring2 ? keyEasing.settlingDuration : getFunctionValue(setValue(key.duration, l > 1 ? getFunctionValue(tDuration, target, ti, tl) / l : tDuration), target, ti, tl);
      ...
      if (toFunc || fromFunc) {
        if (fromFunc) {
          ...
          if (fromTargetObject.u !== tween._unit && tween.target[isDomSymbol]) {
            convertValueUnit(tween.target, fromTargetObject, tween._unit, true);
          }
...
var Timeline = class extends Timer {
  ...
  sync(synced, position) {
    ...
    if (!isUnd(synced) && !isUnd(synced.persist)) {
      synced.persist = true;
    }
...
var remove = (targets, renderable, propertyName) => {
  ...
  for (let i = 0, l = targetsArray.length; i < l; i++) {
    removeWAAPIAnimation(
      targetsArray[i],
      propertyName,
      renderable && renderable.controlAnimation && renderable
    );
  }
...
var Draggable = class {
  constructor(target, parameters = {}) {
    ...
    const xProp = isObj(paramX) && !isUnd(paramX.mapTo) ? paramX.mapTo : "translateX";
    const yProp = isObj(paramY) && !isUnd(paramY.mapTo) ? paramY.mapTo : "translateY";
  ...
  refresh() {
    ...
    const containerPadding = isArr(cp) ? cp : [cp, cp, cp, cp];
    const cx = this.x;
    const cy = this.y;
  ...
  handleDown(e) {
    ...
    if (this.grabbed || $eTarget.type === "range")
      return;
    e.stopPropagation();
    this.grabbed = true;
    this.released = false;
...
var Scope = class {
  constructor(parameters = {}) {
    ...
    let root = doc;
    if (rootParam) {
      root = rootParam.current || rootParam.nativeElement || parseTargets(rootParam)[0] || doc;
    }
...
var ScrollObserver = class {
  constructor(parameters = {}) {
    ...
    const syncMethods = isMethods ? syncMode.split(" ").map(
      (m) => () => {
  ...
  link(linked) {
    if (linked) {
      linked.pause();
      this.linked = linked;
      if (!isUnd(linked) && !isUnd(linked.persist)) {
        linked.persist = true;
      }
    ...
    if (shouldSeek) {
      ...
      if (syncSmooth && isNum(syncSmooth)) {
        if (syncSmooth < 1) {
          ...
          p = round(lerp(lp, p, lerp(0.01, 0.2, syncSmooth)) + snap3, 6);
        }
      } else if (syncEase) {
        p = syncEase(p);
      }
...
var parseWAAPIEasing = (ease) => {
  ...
  if (isStr(ease)) {
    ...
    parsedEase = easingToLinear(ease.ease);
  }
  return parsedEase;
};
...
var WAAPIAnimation = class {
  constructor(targets, params) {
    ...
    parsedTargets.forEach(($el, i) => {
      ...
      const keyEasing = isFnc(easeFunctionResult) || isStr(easeFunctionResult) ? easeFunctionResult : easeToParse;
      const spring2 = easeToParse.ease && easeToParse;
      const easing = parseWAAPIEasing(keyEasing);
      const duration = (spring2 ? spring2.settlingDuration : getFunctionValue(setValue(params.duration, globals.defaults.duration), $el, i, targetsLength)) * timeScale;
...
var updateNodeTimingParams = (node, params) => {
  ...
  const hasSpring = !isUnd(keyEasing) && !isUnd(keyEasing.ease);
...
var roundPad2 = makeChainable("roundPad", numberUtils.roundPad);
var padStart2 = makeChainable("padStart", numberUtils.padStart);
var padEnd2 = makeChainable("padEnd", numberUtils.padEnd);
var wrap2 = makeChainable("wrap", numberUtils.wrap);
var mapRange2 = makeChainable("mapRange", numberUtils.mapRange);
var degToRad2 = makeChainable("degToRad", numberUtils.degToRad);
var radToDeg2 = makeChainable("radToDeg", numberUtils.radToDeg);
var snap2 = makeChainable("snap", numberUtils.snap);
var clamp2 = makeChainable("clamp", numberUtils.clamp);
var round2 = makeChainable("round", numberUtils.round);
var lerp2 = makeChainable("lerp", numberUtils.lerp, 1);
var damp2 = makeChainable("damp", numberUtils.damp, 1);
...
var stagger = (val, params = {}) => {
  ...
  const staggerEase = hasSpring ? ease.ease : hasEasing ? parseEase(ease) : null;
